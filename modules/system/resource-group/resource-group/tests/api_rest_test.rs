...
async fn rest_put_type_returns_200() {
    ...
    let tenant_id = Uuid::now_v7();
    let code = format!(
        "gts.x.system.rg.type.v1~test.put.{}.v1~",
        Uuid::now_v7().as_simple()
    );
...
async fn rest_put_type_not_found_returns_404() {
    let (router, _) = build_test_router().await;
    let tenant_id = Uuid::now_v7();
    let code = "gts.x.system.rg.type.v1~nonexistent.put.v1~";
    let encoded = code.replace('~', "%7E");
...
async fn rest_post_membership_returns_201() {
    ...
    let mt_code = format!(
        "gts.x.system.rg.type.v1~test.mt2.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
    ...
    let gt_code = format!(
        "gts.x.system.rg.type.v1~test.gt2.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn rest_delete_membership_returns_204() {
    ...
    let mt = format!(
        "gts.x.system.rg.type.v1~test.mtr.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
    ...
    let gt = format!(
        "gts.x.system.rg.type.v1~test.gtr.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn rest_post_group_with_parent_returns_201() {
    ...
    let root_type = format!(
        "gts.x.system.rg.type.v1~test.rtp.{}.v1~",
        Uuid::now_v7().as_simple()
    );
...
async fn rest_create_type_with_metadata_schema() {
    ...
    let tenant_id = Uuid::now_v7();
    let code = format!(
        "gts.x.system.rg.type.v1~test.ms.{}.v1~",
        Uuid::now_v7().as_simple()
    );
...
async fn rest_create_group_with_metadata() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.gm.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn rest_group_response_omits_null_metadata() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.nm.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn rest_type_response_omits_null_metadata_schema() {
    ...
    let tenant_id = Uuid::now_v7();
    let code = format!(
        "gts.x.system.rg.type.v1~test.nms.{}.v1~",
        Uuid::now_v7().as_simple()
    );
...
async fn input_membership_non_gts_resource_type() {
    ...
    let rt = format!(
        "gts.x.system.rg.type.v1~test.ngts.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn input_deser_type_can_be_root_string_returns_error() {
    ...
    let req = json_request(
        ...
        "/types-registry/v1/types",
        Some(serde_json::json!({
            "code": "gts.x.system.rg.type.v1~test.deser.v1~",
            "can_be_root": "yes",
            "allowed_parents": [],
            "allowed_memberships": []
        })),
...
async fn input_deser_type_missing_can_be_root_returns_error() {
    ...
    let req = json_request(
        ...
        "/types-registry/v1/types",
        Some(serde_json::json!({
            "code": "gts.x.system.rg.type.v1~test.missing.v1~"
        })),
        tenant_id,
    );
    let resp = router.oneshot(req).await.unwrap();
...
async fn input_deser_group_invalid_parent_uuid_returns_error() {
    ...
    let req = json_request(
        ...
        "/resource-group/v1/groups",
        Some(serde_json::json!({
            "type": "gts.x.system.rg.type.v1~test.v1~",
            "name": "BadParent",
...
async fn input_deser_group_empty_name_returns_400() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.en.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn input_deser_membership_path_non_uuid_returns_400() {
    ...
    let req = json_request(
        "POST",
        "/resource-group/v1/memberships/not-a-uuid/gts.x.system.rg.type.v1~test.v1~/res-001",
        None,
        tenant_id,
    );
    let resp = router.oneshot(req).await.unwrap();
...
async fn input_deser_extra_fields_behavior() {
    ...
    let tenant_id = Uuid::now_v7();
    let code = format!(
        "gts.x.system.rg.type.v1~test.extra.{}.v1~",
        Uuid::now_v7().as_simple()
    );
...
async fn gts_membership_post_tilde_encoded() {
    ...
    let mt = format!(
        "gts.x.system.rg.type.v1~test.tmt.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
    ...
    let gt = format!(
        "gts.x.system.rg.type.v1~test.tgt.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn gts_membership_delete_tilde_encoded() {
    ...
    let mt = format!(
        "gts.x.system.rg.type.v1~test.tmd.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
    ...
    let gt = format!(
        "gts.x.system.rg.type.v1~test.tgd.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn gts_put_type_tilde_encoded() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.tput.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn smallint_type_response_has_no_surrogate_ids() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.sid.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn smallint_group_response_has_no_surrogate_ids() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.gsid.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn smallint_membership_response_has_no_surrogate_ids() {
    ...
    let mt = format!(
        "gts.x.system.rg.type.v1~test.msid.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
    ...
    let gt = format!(
        "gts.x.system.rg.type.v1~test.gsidm.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
...
async fn rest_error_responses_have_problem_content_type_and_status() {
    ...
    let code = format!(
        "gts.x.system.rg.type.v1~test.errdup.{}.v1~",
        Uuid::now_v7().as_simple()
    );
    type_svc
        .create_type(resource_group_sdk::CreateTypeRequest {
    ...
    let req = json_request(
        ...
        "/resource-group/v1/groups",
        Some(serde_json::json!({
            "type": "gts.x.system.rg.type.v1~nonexistent.v1~",
            "name": "Ghost"
        })),
        tenant_id,
    );
...
async fn rest_route_smoke_all_endpoints_registered() {
    ...
    let tenant_id = Uuid::now_v7();
    let fake_id = Uuid::now_v7();
    let fake_code = "gts.x.system.rg.type.v1~smoke.v1%7E";
