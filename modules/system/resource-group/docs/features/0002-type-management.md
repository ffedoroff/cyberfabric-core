...
- [ ] `p1` - **ID**: `cpt-cf-resource-group-featstatus-type-management-implemented`
...
- [ ] `p1` - **ID**: `cpt-cf-resource-group-flow-type-mgmt-create-type`
...
**Steps**:
1. [ ] - `p1` - Actor sends POST /api/types-registry/v1/types with type definition payload - `inst-create-type-1`
2. [ ] - `p1` - Validate GTS type path format via `GtsTypePath` value object - `inst-create-type-2`
3. [ ] - `p1` - Validate placement invariant: `can_be_root OR len(allowed_parents) >= 1` - `inst-create-type-3`
4. [ ] - `p1` - **IF** allowed_parents is non-empty - `inst-create-type-4`
   1. [ ] - `p1` - DB: SELECT id FROM gts_type WHERE schema_id IN (allowed_parents) — verify all referenced parent types exist - `inst-create-type-4a`
   2. [ ] - `p1` - **IF** any parent type not found → **RETURN** Validation error with missing type paths - `inst-create-type-4b`
5. [ ] - `p1` - **IF** allowed_memberships is non-empty - `inst-create-type-5`
   1. [ ] - `p1` - DB: SELECT id FROM gts_type WHERE schema_id IN (allowed_memberships) — verify all referenced membership types exist - `inst-create-type-5a`
   2. [ ] - `p1` - **IF** any membership type not found → **RETURN** Validation error with missing type paths - `inst-create-type-5b`
6. [ ] - `p1` - Resolve GTS type path to SMALLINT surrogate ID at persistence boundary - `inst-create-type-6`
7. [ ] - `p1` - DB: INSERT INTO gts_type (schema_id, metadata_schema) — with uniqueness constraint on schema_id - `inst-create-type-7`
8. [ ] - `p1` - **IF** unique constraint violation → **RETURN** TypeAlreadyExists with conflicting schema_id - `inst-create-type-8`
9. [ ] - `p1` - DB: INSERT INTO gts_type_allowed_parent (type_id, parent_type_id) for each allowed parent - `inst-create-type-9`
10. [ ] - `p1` - DB: INSERT INTO gts_type_allowed_membership (type_id, membership_type_id) for each allowed membership - `inst-create-type-10`
11. [ ] - `p1` - **RETURN** created ResourceGroupType with schema_id, allowed_parents, allowed_memberships, can_be_root, metadata_schema - `inst-create-type-11`

### Update Type

- [ ] `p1` - **ID**: `cpt-cf-resource-group-flow-type-mgmt-update-type`
...
**Steps**:
1. [ ] - `p1` - Actor sends PUT /api/types-registry/v1/types/{code} with updated definition - `inst-update-type-1`
2. [ ] - `p1` - DB: SELECT FROM gts_type WHERE schema_id = {code} — load existing type - `inst-update-type-2`
3. [ ] - `p1` - **IF** type not found → **RETURN** NotFound - `inst-update-type-3`
4. [ ] - `p1` - Validate placement invariant on new values - `inst-update-type-4`
5. [ ] - `p1` - Validate all referenced allowed_parents and allowed_memberships types exist - `inst-update-type-5`
6. [ ] - `p1` - Invoke hierarchy safety check algorithm for allowed_parents and can_be_root changes - `inst-update-type-6`
7. [ ] - `p1` - **IF** hierarchy safety check fails → **RETURN** AllowedParentsViolation with violating group details - `inst-update-type-7`
8. [ ] - `p1` - DB: DELETE FROM gts_type_allowed_parent WHERE type_id = {id} — clear old parents - `inst-update-type-8`
9. [ ] - `p1` - DB: INSERT INTO gts_type_allowed_parent — insert new parents - `inst-update-type-9`
10. [ ] - `p1` - DB: DELETE FROM gts_type_allowed_membership WHERE type_id = {id} — clear old memberships - `inst-update-type-10`
11. [ ] - `p1` - DB: INSERT INTO gts_type_allowed_membership — insert new memberships - `inst-update-type-11`
12. [ ] - `p1` - DB: UPDATE gts_type SET metadata_schema = {new}, updated_at = now() - `inst-update-type-12`
13. [ ] - `p1` - **RETURN** updated ResourceGroupType - `inst-update-type-13`

### Delete Type

- [ ] `p1` - **ID**: `cpt-cf-resource-group-flow-type-mgmt-delete-type`
...
**Steps**:
1. [ ] - `p1` - Actor sends DELETE /api/types-registry/v1/types/{code} - `inst-delete-type-1`
2. [ ] - `p1` - DB: SELECT id FROM gts_type WHERE schema_id = {code} - `inst-delete-type-2`
3. [ ] - `p1` - **IF** type not found → **RETURN** NotFound - `inst-delete-type-3`
4. [ ] - `p1` - DB: SELECT COUNT(*) FROM resource_group WHERE gts_type_id = {type_id} - `inst-delete-type-4`
5. [ ] - `p1` - **IF** count > 0 → **RETURN** ConflictActiveReferences with entity count - `inst-delete-type-5`
6. [ ] - `p1` - DB: DELETE FROM gts_type WHERE id = {type_id} — CASCADE deletes junction table rows - `inst-delete-type-6`
7. [ ] - `p1` - **RETURN** success (204 No Content) - `inst-delete-type-7`

## 3. Processes / Business Logic (CDSL)

### Type Input Validation

- [ ] `p1` - **ID**: `cpt-cf-resource-group-algo-type-mgmt-validate-type-input`
...
**Steps**:
1. [ ] - `p1` - Validate `schema_id` via GtsTypePath value object (format, length, non-empty) - `inst-val-input-1`
2. [ ] - `p1` - **IF** `schema_id` does not have RG type prefix `gts.x.system.rg.type.v1~` - `inst-val-input-2`
   1. [ ] - `p1` - **RETURN** Validation error: "Type schema_id must have RG type prefix" - `inst-val-input-2a`
3. [ ] - `p1` - Validate placement invariant: `can_be_root == true OR len(allowed_parents) >= 1` - `inst-val-input-3`
4. [ ] - `p1` - **IF** invariant violated - `inst-val-input-4`
   1. [ ] - `p1` - **RETURN** Validation error: "Type must allow root placement or have at least one allowed parent" - `inst-val-input-4a`
5. [ ] - `p1` - **FOR EACH** parent_path in allowed_parents - `inst-val-input-5`
   1. [ ] - `p1` - Validate parent_path has RG type prefix `gts.x.system.rg.type.v1~` - `inst-val-input-5a`
   2. [ ] - `p1` - Verify parent_path exists in gts_type table - `inst-val-input-5b`
6. [ ] - `p1` - **FOR EACH** membership_path in allowed_memberships - `inst-val-input-6`
   1. [ ] - `p1` - Validate membership_path is a valid GtsTypePath (no RG prefix requirement) - `inst-val-input-6a`
   2. [ ] - `p1` - Verify membership_path exists in gts_type table - `inst-val-input-6b`
7. [ ] - `p1` - **IF** metadata_schema provided, validate it is valid JSON Schema - `inst-val-input-7`
8. [ ] - `p1` - **RETURN** validated type definition - `inst-val-input-8`

### Hierarchy Safety Check for Type Update

- [ ] `p1` - **ID**: `cpt-cf-resource-group-algo-type-mgmt-check-hierarchy-safety`
...
**Steps**:
1. [ ] - `p1` - Compute removed parent types: `old_allowed_parents - new_allowed_parents` - `inst-hier-check-1`
2. [ ] - `p1` - **FOR EACH** removed_parent_type in removed set - `inst-hier-check-2`
   1. [ ] - `p1` - DB: SELECT rg.id, rg.name FROM resource_group rg JOIN resource_group parent ON rg.parent_id = parent.id WHERE rg.gts_type_id = {this_type_id} AND parent.gts_type_id = {removed_parent_type_id} - `inst-hier-check-2a`
   2. [ ] - `p1` - **IF** any groups found → collect as violations - `inst-hier-check-2b`
3. [ ] - `p1` - **IF** can_be_root changed from true to false - `inst-hier-check-3`
   1. [ ] - `p1` - DB: SELECT id, name FROM resource_group WHERE gts_type_id = {this_type_id} AND parent_id IS NULL - `inst-hier-check-3a`
   2. [ ] - `p1` - **IF** any root groups found → collect as violations - `inst-hier-check-3b`
4. [ ] - `p1` - **IF** violations collected → **RETURN** AllowedParentsViolation with violating group IDs, names, and constraint details - `inst-hier-check-4`
5. [ ] - `p1` - **RETURN** pass - `inst-hier-check-5`
...
- [ ] `p1` - **ID**: `cpt-cf-resource-group-dod-type-mgmt-service-crud`
...
- [ ] `p1` - **ID**: `cpt-cf-resource-group-dod-type-mgmt-rest-handlers`
