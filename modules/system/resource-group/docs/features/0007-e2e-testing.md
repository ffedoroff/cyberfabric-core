...
End-to-end test plan for the resource-group module. Covers 10 integration seam tests using real PostgreSQL, HTTP, and AuthN/AuthZ pipeline. Complements 325 unit/integration tests from Feature 0006.

### Purpose

Verify integration seams that unit tests (Feature 0006, 318 tests) cannot cover: HTTP routing, PostgreSQL-specific behavior, real AuthN/AuthZ, MTLS, and cursor codec over HTTP.
...
This is not "another way to verify business logic". Business logic is already verified in Feature 0006 (325 unit/integration tests on SQLite in-memory — 308 in `cf-resource-group`, 16 in `cf-resource-group-sdk`). E2E tests verify the **seams between components**, not the components themselves.
...
- GTS type path validation (0006: 10 tests in `models.rs` on `GtsTypePath`)
...
- All `DomainError` variants (0006: 79 tests in `domain_unit_test.rs`)
- DTO struct↔struct conversions (0006: inline tests in `dto.rs`)
- OData filter field names and kinds (0006: 5 tests in SDK `odata/*.rs`)
- Metadata schema round-trip and attack vectors (0006: 45 tests in `type_service_test.rs`)
- Seeding idempotency (0006: 12 tests in `seeding_test.rs`)
- AccessScope construction (0006: `tenant_scoping_test.rs`, 10 tests)
- Group service PATCH logic (0006: TC-GRP-11a/11b in `group_service_test.rs`)
...
**Fidelity** — real PostgreSQL, real AuthZ pipeline, real HTTP. This is the only thing that justifies E2E on top of 325 unit tests.
...
|---|------|------|--------------|---------------|
| S1 | Route smoke | Route registration | 0006 has 54 REST tests via `Router::oneshot` including route smoke for all 15 endpoints | All 15 endpoints respond non-405 on real server |
| S2 | DTO roundtrip | Serialization | dto.rs inline tests cover struct conversion; not JSON wire | Full JSON shape matches OpenAPI |
| S3 | AuthZ tenant filter | AuthZ → SecureORM | authz_integration_test (9 tests) mocks PDP | Real PolicyEnforcer + real DB + real WHERE |
| S4 | Cross-tenant isolation | AuthZ → SecureORM | tenant_filtering_db_test (7 tests) uses SQLite | Real PostgreSQL + real tokens + HTTP 404 |
| S5 | Hierarchy + closure (PG) | PostgreSQL closure SQL | TC-GRP-01/02 on SQLite (55 tests total) | Real PG INSERT INTO...SELECT, correct depths |
...
| S8 | Error response format | Error middleware | domain_unit_test (79 tests) checks DomainError→Problem; TC-REST-10 (api_rest_test, 53 tests) covers Content-Type via Router::oneshot | HTTP Content-Type header on real server |
| S9 | Cursor pagination | Pagination codec | None (0006 tests page logic, not codec) | Encode→HTTP→decode roundtrip, no duplicates |
| S10 | Membership filter | OData filter wiring | SDK OData field tests (5 tests) + all 4 OData mappers tested (4 tests) | Real $filter parse → SQL WHERE → correct subset |
...
PATCH /cf/resource-group/v1/groups/{uuid}       → not 405
...
Minimal positive scenario — complex move logic (grandchild, subtree preservation) is already covered by 0006 TC-GRP-05 on SQLite. E2E only verifies the PG-specific SQL executes without error and produces correct depth.
...
Minimal positive scenario — cascade completeness (memberships, grandchildren) is already covered by 0006 TC-GRP-15 on SQLite. E2E only verifies the deletion order doesn't violate PG FK constraints.
...
**Why not in 0006**: 0006 TC-REST-10 covers multiple error codes via `Router::oneshot`, but uses mocked AuthZ and SQLite. E2E verifies the real server process serializes errors correctly with real middleware chain. One error path suffices — the middleware is uniform.

Minimal positive scenario — multiple error codes and RFC 9457 field verification are already covered by 0006 TC-REST-10. E2E only verifies the real server middleware chain produces correct Content-Type.
...
|---|---|---|
| Type validation (invalid GTS path, placement invariant) | Pure domain logic, deterministic | 0006: type_service_test (45 tests), models.rs (10 tests) |
| Cycle detection logic | Pure domain logic on closure table | 0006: TC-GRP-06/07 in group_service_test (55 tests) |
...
| Membership tenant compatibility | Domain logic with mock tenants | 0006: TC-MBR-06/10/14 in membership_service_test (15 tests) |
| Seeding idempotency | Domain logic, no HTTP | 0006: seeding_test (12 tests) |
| Metadata schema round-trip + attack vectors | Pure domain + DB logic | 0006: type_service_test (TC-META-*, TC-META-ATK-*) |
| Individual error variants (DomainError types) | Error construction + mapping | 0006: domain_unit_test (79 tests) |
| AccessScope construction | Pure logic | 0006: tenant_scoping_test (10 tests) |
| InGroup / InGroupSubtree predicates | Compiler + SecureORM | 0006: tenant_filtering_db_test (7 tests) |
| Group PATCH (partial update) service logic | Domain logic | 0006: TC-GRP-11a/11b in group_service_test |
...
- [x] S7 (force delete) verifies PG FK cascade succeeds — 204 + target 404
...
- [x] No test duplicates 0006 domain logic (318 tests) — if removing the test doesn't reduce integration confidence, the test shouldn't exist
