...
Unit and integration test plan for the resource-group module. Covers ~325 tests across domain services, value objects, error chains, DTOs, OData fields, seeding, and REST API layer using SQLite in-memory and mocked AuthZ.
...
- [4. Priority Matrix](#4-priority-matrix)
  - [P1 - Critical (must have, business invariants) — 64 tests](#p1---critical-must-have-business-invariants--64-tests)
...
- [5. Assert Guidelines — What to Verify Beyond Ok/Err](#5-assert-guidelines--what-to-verify-beyond-okerr)
  - [5.1 Closure Table Assertions (`resource_group_closure`)](#51-closure-table-assertions-resource_group_closure)
  - [5.2 Junction Table Assertions (`gts_type_allowed_parent`, `gts_type_allowed_membership`)](#52-junction-table-assertions-gts_type_allowed_parent-gts_type_allowed_membership)
  - [5.3 Membership Table Assertions (`resource_group_membership`)](#53-membership-table-assertions-resource_group_membership)
  - [5.4 Surrogate ID Non-Exposure (REST tests)](#54-surrogate-id-non-exposure-rest-tests)
  - [5.5 Entity State Assertions (`resource_group` table)](#55-entity-state-assertions-resource_group-table)
...
- [Acceptance Criteria](#acceptance-criteria)
  - [Remaining Gaps (see section 2.2 — RG1..RG7)](#remaining-gaps-see-section-22--rg1rg7)
...
~325 tests (302 in `cf-resource-group` + 16 in `cf-resource-group-sdk`). Fast (< 5s total). Zero sleeps. Every test atomic.
...
| `api_rest_test.rs` | 54 | Type CRUD REST (create 201, dup 409, invalid 400, list 200, get 200/404, delete 204), Group REST (create/list/get/update/delete/hierarchy/PATCH), Membership REST (add/remove/list), RFC 9457 error format + Content-Type verification (TC-REST-10), deserialization errors, SMALLINT non-exposure, metadata in REST responses, PATCH partial update (TC-REST-09), route smoke all 15 endpoints (RG7) |
...
| `group_service_test.rs` | 55 | TC-GRP-01..38: child creation + closure rows, 3-level hierarchy, incompatible parent type, can_be_root enforcement, move with closure rebuild, cycle detection, self-parent, type change validation, partial update (PATCH), leaf/force delete, hierarchy depth traversal, max_depth/max_width enforcement, name validation, cross-tenant parent, simultaneous type+parent change, detach to root, metadata barrier tests |
...
| G15a-G15c | Group Patch | Partial update (PATCH) at service level | ✅ **CLOSED** — tested in `group_service_test.rs` (TC-GRP-11a, TC-GRP-11b) |
| G25-G33 | Membership | Add/remove, validation, tenant compat, duplicates | ✅ **CLOSED** — 15 tests in `membership_service_test.rs` |
| G34-G35 | REST Layer | Update type, membership endpoints | ✅ **CLOSED** — 53 tests in `api_rest_test.rs` (PUT type, POST/DELETE membership, hierarchy, force delete, metadata, SMALLINT non-exposure, deserialization errors, GTS tilde encoding, PATCH groups, error Content-Type) |
...
|---|----------|------|-----|--------|
| ~~RG1~~ | ~~CRITICAL~~ | ~~REST: PATCH Groups~~ | ✅ **CLOSED** — 4 tests added to `api_rest_test.rs` (TC-REST-09): name-only update, null parent detach, empty body noop, 404 not found | |
...
| ~~RG7~~ | ~~MEDIUM~~ | ~~REST: Route Registration~~ | ✅ **CLOSED** — `rest_route_smoke_all_endpoints_registered` test verifies all 15 endpoints (5 type + 7 group + 3 membership) respond non-405 via `Router::oneshot`. | |
...
- POST `/memberships/{group_id}/<url-encoded-gts-type-path>/res-1`
...
#### TC-REST-09: PATCH group via REST — partial update [P1]
- **Covers**: RG1, cpt-cf-resource-group-fr-partial-update-group
- **Why**: PATCH /groups/{id} is the only REST endpoint with zero HTTP-level tests. Service-level PATCH logic is verified by TC-GRP-11a/11b, but HTTP routing, `Option<Option<T>>` deserialization (absent vs explicit null), and response shape are untested.
- **Setup**: Create type + group via service. Send PATCH with `{"name": "patched"}` (only name, no other fields).
- **Assert**: 200 OK, response body has updated `name`, other fields unchanged (metadata, parent_id, type)
- **Negative**: PATCH with `{"parent_id": null}` → 200, group becomes root (parent_id cleared). PATCH with empty body `{}` → 200, nothing changed. PATCH nonexistent group → 404.
...
| **TC-REST-09** | PATCH group via REST — partial update | PATCH endpoint untested at HTTP level; deserialization of `Option<Option<T>>` unverified |
...
fn gts_type_path_invalid_cases() {
    let invalid = vec![
        ...
        ("<valid-prefix-without-tilde>", "no trailing tilde"),
    ];
...
tests/
  ...
  api_rest_test.rs               # ✅ DONE (54 tests) — includes PATCH (TC-REST-09), error Content-Type (TC-REST-10), route smoke (RG7)
...
### Remaining Gaps (see section 2.2 — RG1..RG7)

- [x] ~~RG1~~: PATCH /groups/{id} REST-level tests added (4 tests)
...
- [x] ~~RG6~~: Accepted — module init is integration-level, covered by E2E (0007)
- [x] ~~RG7~~: Route smoke test added — all 15 endpoints verified non-405
