...
- `PageInfo` — pagination metadata with `next_cursor`, `prev_cursor`, `limit`, `has_next_page`, `has_previous_page`
...
**Required traits**:
- `ResourceGroupClient` — full CRUD trait: type management (`create_type`, `get_type`, `list_types`, `update_type`, `delete_type`), group management (`create_group`, `get_group`, `list_groups`, `update_group`, `patch_group`, `delete_group`, `list_group_depth`), membership management (`add_membership`, `remove_membership`, `list_memberships`). All methods accept `SecurityContext` as first argument.
