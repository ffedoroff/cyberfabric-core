...
AuthZ resolver plugin that resolves tenant hierarchy via `TenantResolverClient`. Replacement for `rg-authz-plugin` — delegates all tenant operations to the tenant-resolver module instead of accessing Resource Group directly.
...
modules:
  tr_authz_plugin:
    config:
      vendor: "hyperspot"
      priority: 50  # Lower than rg-authz-plugin (200) to take precedence
```
