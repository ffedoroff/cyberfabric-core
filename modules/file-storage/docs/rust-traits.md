...
pub enum BackendKind {
    ...
    /// local-disk recipe — see DESIGN §4 "Testing strategy").
    /// Metadata in the shared FileStorage database (per ADR-0001).
    S3Compatible,
}

/// Transport mode for the byte plane. P1 ships only `Redirect`;
/// every byte transfer is client ↔ storage backend direct via a
/// presigned URL handed back by FileStorage (or, for public-read
/// backends, a bare-HTTPS URL — still client ↔ backend direct).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BackendTransport {
    /// Bytes flow client ↔ backend directly via presigned URLs.
    Redirect,
}
...
pub enum BackendCapability {
    ...
    /// expiry — it is eternal for the file's lifetime.
    PublicReadUrls,
}
...
pub trait FileStorageClient: Send + Sync {
    ...
    ///
    /// **Fully implemented in P1.** This is NOT a proxy over REST and
    /// does NOT mint a presigned URL: in-process callers share the
    /// FileStorage runtime, so the SDK drives the adapter directly
    /// (e.g. `aws-sdk-s3 PutObject` for `s3-compatible` backends) and
    /// commits the row in the same call. The REST proxy path
    /// (bytes-through-FileStorage on `POST /files`) is the separate P3
    /// concern reserved for HTTP clients that cannot speak the
    /// presign-first protocol — see DESIGN §4 Future deltas. External
    /// frontends in P1 use the `create_presigned_url` → external PUT
    /// → `reconcile` triad; `put_file` is the in-process equivalent
    /// that compresses the same lifecycle into one async call.
    ///
    /// **Lifecycle (`s3-compatible`).**
...
The adapter trait is the seam between the FileStorage core (which owns the `file_storage` SQL schema, the upload coordinator, and authz integration) and a concrete byte store. It deliberately does **not** know about `file_id`, `OwnerRef`, or authz — those are resolved one layer up. The adapter only ever sees a `BackendObjectKey` (an opaque `String` minted by the core) and bytes. It also does NOT know about `gts_file_type` — that field is DB-only and never crosses the adapter boundary.
...
|-------|---------|-----------|---------------|
| `FileStorageClient` | 12 (full P1 surface: backends, file lifecycle including `reconcile`, both initial and variant-B presign, streaming I/O including the in-process direct-upload `put_file`, batched presign) | CyberFabric modules via ClientHub, REST adapter | `dyn FileStorageClient` |
| `StorageBackend` (internal) | 6 (adapter boundary; `head_object` is required by `reconcile` and the strong-CAS variant of `put_file_info`; `copy_object_self` is required by `put_file_info`'s DB+S3 sync; presigned URLs are batch-first on the GET side, single-shot on the PUT side per ADR-0003) | `BackendRouter`, `UploadCoordinator` | — (not registered in ClientHub) |
